...
public final class Board extends Observable {
  ...
  /** The next ID number available when instantiating new Boards. */
  private static int my_next_id;
  
  /**
   * The ID of this Tetris board. 
   */
  ...
  public Board(final int the_width,
    ...
    // newGame(the_width, the_height, the_pieces); //removed
  }
  ...
  public void newGame(final int the_width, final int the_height,
    ...
    if (the_width < MIN_SIZE || the_height < MIN_SIZE) {
      throw new IllegalArgumentException("cannot define a play area less than " + MIN_SIZE
                                         + "in a dimension.");
    }
  ...
  /**
   * Returns the ghost of the current Piece.
   * 
   * @return the ghost of the current Piece.
   */
  public MutablePiece getGhostPiece() {
    MutablePiece ghost = null;
    try {
      ghost = (MutablePiece) ((AbstractPiece) my_current_piece).clone();
    } catch (final CloneNotSupportedException e) {
      e.printStackTrace();
    }
    ((AbstractPiece) ghost).turnIntoGhostPiece();

    boolean can_pass = canMoveDown(ghost);
    while (can_pass) {
      ghost.moveDown();
      can_pass = canMoveDown(ghost);
    }
    return ghost;
  }
   ...
   *         otherwise
   */
  public boolean moveDown() { //edited
    final boolean can_pass = canMoveDown(my_current_piece);
   ...
   * 
   * @param the_piece the piece to check for.
   * @return true if it is possible to move the piece piece down; false
   *         otherwise
   */
  private boolean canMoveDown(final MutablePiece the_piece) { //new method
    final int[][] blocks = ((AbstractPiece) the_piece).getBoardCoordinates();
    boolean can_pass = true;
   ...
   * 
   * @param the_cc true if the piece should be rotated clockwise; false if the
   *          piece should be rotated counterclockwise.
   * @return true if it is possible to rotate the current piece; false otherwise
   */
  public boolean rotate(final boolean the_cc) { // changed
    my_current_piece.rotate(the_cc);
    final int[][] blocks = ((AbstractPiece) my_current_piece).getBoardCoordinates();
    boolean can_pass = true;
  ...
  /**
   * Sets the board into a game over state.
   */
  public void endGame() { //new method
    my_game_is_over = true;
    freezePiece();
    for (final Block[] blocks : my_blocks) {
      for (int i = 0; i < my_width; i++) {
        if (blocks[i] != Block.EMPTY) {
          blocks[i] = Block.JUNK;
        } else {
          blocks[i] = Block.PERMANENT;
        }
      }
    }
  }
   ...
   * <p>
   * This method should be called to end the game if the player meets the conditions for
   * a loss.  Otherwise, invoke the <code>endGame()</code> method instead.
   */
  private void gameOver() { //new method
    endGame();
    setChanged();
    notifyObservers(my_game_is_over);
  }
   ...
   */
  private void freeze() { //edited
    freezePiece();
    clearLines();
    if (my_current_piece.getY() > my_height - EXTRA_ROWS) { //changed to conform to specs
      gameOver();
    }
    assignCurrentPiece();
    setChanged();
  ...
  /**
   * Freezes the current piece in place.
   */
  private void freezePiece() { // new method
  ...
  private void clearLines() {
    ...
    setChanged();
    notifyObservers(lines_cleared);
  }
  
  /**
   * Adds garbage lines to the board.  Used for multiplayer.
   * 
   * @param the_lines the number of garbage lines to be added.
   */
  public void addGarbage(final int the_lines) { //new method for multiplayer matches
    final int n = RANDOM.nextInt(my_width);
    for (int i = 0; i < the_lines; i++) {
      final Block[] blocks = new Block[my_width];
      for (int j = 0; j < my_width; j++) {
        if (j == n) {
          blocks[j] = Block.EMPTY;
        } else {
          blocks[j] = Block.JUNK;
        }
      }
      my_blocks.add(0, blocks);
  ...
  /**
   * Clears the board of blocks and repopulates them with <code>EMPTY</code>
   * blocks.
   */
  private void clearBoard() { //new method
    my_blocks.clear();
    fillBoard();
  }

  /**
   * Populates missing rows with <code>EMPTY</code> blocks.
   */
  private void fillBoard() { //new method
    while (my_height + EXTRA_ROWS >= my_blocks.size()) {
