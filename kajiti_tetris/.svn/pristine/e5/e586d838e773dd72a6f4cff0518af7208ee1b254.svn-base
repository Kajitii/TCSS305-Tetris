...
public interface MutablePiece {
  ...
  //included so that an opposing player's piece doesn't get "mushed" into a board's
  //frozen blocks.  Wouldn't that be awkward?
  /** Shifts the piece one space up. */
  void moveUp(); 

  /** 
   * Rotates the piece one quarter turn clockwise.
   * @param the_cc true if the piece should be rotated clockwise; false if the piece
   * should be rotated counterclockwise.
   */
  void rotate(final boolean the_cc); //altered for extra functionality
